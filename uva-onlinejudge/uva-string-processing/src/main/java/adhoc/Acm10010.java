...
public class Acm10010 {
	...
	static int[] findString(char[][] grid, String string) {
		for (int row = 0; row < grid.length; row++) {
			for (int col = 0; col < grid[row].length; col++) {
				if (grid[row][col] == string.charAt(0)) { // char in this grid position is the same as the first
															// character of the string
					if (checkEast(grid, string, row, col) || checkNorth(grid, string, row, col)
							|| checkNorthEast(grid, string, row, col) || checkNorthWest(grid, string, row, col)
							|| checkSouth(grid, string, row, col) || checkSouthEast(grid, string, row, col)
							|| checkSouthWest(grid, string, row, col) || checkWest(grid, string, row, col))
	...
	static boolean checkNorth(char[][] grid, String string, int row, int col) {
		int i = 0;
		for (; row >= 0 && i < string.length() && grid[row][col] == string.charAt(i); row--, i++)
			;
	...
	static boolean checkSouth(char[][] grid, String string, int row, int col) {
		int i = 0;
		for (; row < grid.length && i < string.length() && grid[row][col] == string.charAt(i); row++, i++)
			;
		return (i == string.length());
	}
	...
	static boolean checkWest(char[][] grid, String string, int row, int col) {
		int i = 0;
		for (; col >= 0 && i < string.length() && grid[row][col] == string.charAt(i); col--, i++)
			;
		return (i == string.length());
	}
	...
	static boolean checkEast(char[][] grid, String string, int row, int col) {
		int i = 0;
		for (; col < grid[row].length && i < string.length() && grid[row][col] == string.charAt(i); col++, i++)
			;
		return (i == string.length());
	}
	...
	static boolean checkNorthEast(char[][] grid, String string, int row, int col) {
		int i = 0;
		for (; row >= 0 && col < grid[row].length && i < string.length()
	...
	static boolean checkNorthWest(char[][] grid, String string, int row, int col) {
		int i = 0;
		for (; row >= 0 && col >= 0 && i < string.length() && grid[row][col] == string.charAt(i); row--, col--, i++)
			;
		return (i == string.length());
	}
